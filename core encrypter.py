from tkinter import *                       #imports all packages from the tkinter
import tkinter.filedialog                   # imports filedialog from tkinter
from tkinter import messagebox    #imports messagebox from tkinter
from PIL import ImageTk                 #imports ImageTK from PIL
from PIL import Image                     #imports Image from PIL
from io import BytesIO                     #imports BytesIO from PIL
import  os

class IMG_Stegno:
    output_image_size = 0   #declares the size of the encrypted image

    def main(self, root):       #properties of the main frames of the GUI
        root.title('CORE ENCRYPTER')
        ...
        title = Label(frame,text='CORE ENCRYPTER')  
        title.config(font=('Times new roman',25, 'bold','underline'))
        title.grid(pady=10)
    ...
    def back(self,frame):   #function to breake the loop and bring back the user to the main screen
        frame.destroy()
        self.main(root)

    def encode_frame1(self,F):  #function for the Encoding page Frame
        F.destroy()
        F2 = Frame(root)
        label1= Label(F2,text='Select the Image to Encode :')
    ...
    def decode_frame1(self,F):   #function for the Decoding page Frame
        F.destroy()
        d_f2 = Frame(root)
        label1 = Label(d_f2, text='Select Image to Decode')
        label1.config(font=('Times new roman',25,'bold'),bg = '#e3f4f1')
        label1.grid()
    ...
    def encode_frame2(self,e_F2):   #function to encode the image with the message
        e_pg= Frame(root)
        myfile = tkinter.filedialog.askopenfilename(filetypes = ([('png', '*.png'),('jpeg', '*.jpeg'),('jpg', '*.jpg'),('All Files', '*.*')]))    #declares the file type to be selected in the image attachment section
        if not myfile:
    ...
    def decode_frame2(self,d_F2):   #function to decode the image 
        d_F3 = Frame(root)
        ...
        if not myfiles:
            ...
            label2 = Label(d_F3, text='Decrypted Message is :')
            label2.config(font=('Helvetica',14,'bold'))
            label2.grid(pady=10)
    ...
    def decode(self, image):   #function to extract the message from the decoded image 
    ...
    def generate_Data(self,data):   #function to generate the data
        new_data = []
    ...
    def modify_Pix(self,pix, data):   #function to modify the pixels of the encoded image 
    ...
    def encode_enc(self,newImg, data):   #function to attach the data pixels with the image pixels 
        w = newImg.size[0]
        (x, y) = (0, 0)
    ...
    def enc_fun(self,text_a,myImg):   #function to enter the secret message
        data = text_a.get("1.0", "end-1c")
        if (len(data) == 0):
            ...
            messagebox.showinfo("Encoding Successful")
